...
- Light and dark are both supported, in six colour themes: Default, Tokyo Night, Catppuccin, Nord, Gruvbox, Rosé Pine. The theme ids and the two storage keys (`pterodactyl:theme` for the theme, `pterodactyl:theme-mode` for light/dark/system) live in `src/lib/theme.ts` + `src/stores/themeStore.ts` and are mirrored in the pre-paint script in `resources/views/templates/app.blade.php` — change one and you must change the other (`src/lib/theme.test.ts` fails if they drift apart).
- A theme is plain CSS: the default in `:root` / `.dark` in `src/styles/globals.css`, the rest in `src/styles/themes.css` as `:root[data-theme='<id>']:not(.dark)` and `:root[data-theme='<id>'].dark`. Both blocks set every token, including the terminal's ANSI palette (`--terminal-ansi-*`, hex so xterm can parse it). Adding a theme is one entry in `THEMES`, one in the pre-paint script and two CSS blocks — nothing else. The terminal (`src/lib/terminalTheme.ts`) and the file editor (`src/lib/editorTheme.ts`) read those tokens, so they follow the active theme for free.
